...
class Tracks extends Table {
  ...
  TextColumn get availableCountries => text().map(const _StringListConverter()).withDefault(const Constant([]))();
...
class Artists extends Table {
  ...
  TextColumn get genres => text().map(const _StringListConverter()).withDefault(const Constant([]))();
  TextColumn get rawData => text().map(const _JsonMapConverter()).nullable()();
  DateTimeColumn get cachedAt => dateTime().withDefault(currentDateAndTime)();
...
class _JsonMapConverter extends TypeConverter<Map<String, dynamic>, String> {
  ...
  @override
  Map<String, dynamic> fromSql(String fromDb) => fromDb.isEmpty ? {} : json.decode(fromDb) as Map<String, dynamic>;
...
class _JsonListConverter extends TypeConverter<List<dynamic>, String> {
  ...
  @override
  List<dynamic> fromSql(String fromDb) => fromDb.isEmpty ? [] : json.decode(fromDb) as List<dynamic>;
...
class _JsonValueConverter extends TypeConverter<dynamic, String> {
  ...
  @override
  dynamic fromSql(String fromDb) => fromDb.isEmpty ? null : json.decode(fromDb);
  
  @override
  String toSql(dynamic value) => json.encode(value);
}
...
class AppDatabase extends _$AppDatabase {
  ...
  // Track operations
  Future<void> cacheTrack(TracksCompanion track) => into(tracks).insertOnConflictUpdate(track);
  Future<List<TrackEntity>> getAllTracks() => select(tracks).get();
  Future<TrackEntity?> getTrack(String id) => (select(tracks)..where((t) => t.id.equals(id))).getSingleOrNull();
  Future<void> markTrackOffline(String id, String filePath) => 
      (update(tracks)..where((t) => t.id.equals(id))).write(TracksCompanion(isOffline: Value(true), offlineFilePath: Value(filePath)));
  Future<void> deleteTrack(String id) => (delete(tracks)..where((t) => t.id.equals(id))).go();

  // Playlist operations
  Future<void> cachePlaylist(PlaylistsCompanion playlist) => into(playlists).insertOnConflictUpdate(playlist);
  Future<List<PlaylistEntity>> getAllPlaylists() => select(playlists).get();
  Future<PlaylistEntity?> getPlaylist(String id) => (select(playlists)..where((p) => p.id.equals(id))).getSingleOrNull();
  ...
  Future<List<TrackEntity>> getPlaylistTracks(String playlistId) => 
      (select(tracks)..join([innerJoin(playlistTracks, playlistTracks.trackId.equalsExp(tracks.id))])
        ..where(playlistTracks.playlistId.equals(playlistId))
        ..orderBy([OrderingTerm.asc(playlistTracks.position)])).get();

  // History operations
  Future<void> addToHistory(String trackId, {int progress = 0, bool completed = false}) =>
      into(history).insert(HistoryCompanion(
        trackId: Value(trackId),
        progress: Value(progress),
        completed: Value(completed),
      ));
  Future<List<TrackEntity>> getHistory({int limit = 50}) =>
      (select(tracks)..join([innerJoin(history, history.trackId.equalsExp(tracks.id))])
        ..orderBy([OrderingTerm.desc(history.playedAt)])
        ..limit(limit)).get();

  // Lyrics cache
  Future<void> cacheLyrics(LyricsCacheCompanion lyrics) => into(lyricsCache).insertOnConflictUpdate(lyrics);
  Future<LyricsCacheEntity?> getCachedLyrics(String trackId) => 
      (select(lyricsCache)..where((l) => l.trackId.equals(trackId))).getSingleOrNull();

  // Search history
  Future<void> addSearchHistory(String query, String filter) =>
      into(searchHistory).insert(SearchHistoryCompanion(query: Value(query), filter: Value(filter)));
  Future<List<SearchHistoryEntity>> getSearchHistory({int limit = 20}) =>
      (select(searchHistory)..orderBy([OrderingTerm.desc(searchHistory.searchedAt)])..limit(limit)).get();
  ...
  Future<dynamic> getSetting(String key) =>
      (select(settings)..where((s) => s.key.equals(key))).getSingleOrNull().then((s) => s?.value);

  // Downloads
  Future<void> addDownload(DownloadsCompanion download) => into(downloads).insert(download);
  Future<void> updateDownloadProgress(String id, int downloaded, int total, {String? status}) =>
      (update(downloads)..where((d) => d.id.equals(id))).write(DownloadsCompanion(
